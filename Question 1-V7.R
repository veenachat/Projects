...
setwd("/Users/veenachatterjee/Desktop/Tilburg_SEM2/DSM_ass_2")
...
Q1_c <- ggplot(finaldata, aes(x = Year, y = AnnualCOEmissionsperCapita, color = Entity)) +
  geom_line(linewidth = 0.5) +
  geom_text(data = max_values, aes(label = Entity), hjust = -0.1, vjust = 0.5, size = 3, color = "black") +
  labs(title = "emissions over time for all countries (Estonia removed)",
       x = "year", y = "CO2 emissions") +
  theme_light() +
  theme(legend.position = "bottom") +
  xlim(x_min, x_max)  
...
# add slight colors for presenting numbers
library(RColorBrewer)
brewer.pal(3, "PuBu")
# choose "#A6BDDB"

pdf(file="Q1_f.pdf",width=10, height=10)
biplot(pr.out.datas, scale=0, expand=1, choices = c(1, 2), col=c("#A6BDDB","black"), main="PCA without standardization")
dev.off() 
...
biplot(pr.stand.datas, scale=0, expand=1, choices = c(1, 2), col=c("#A6BDDB","black"), main="PCA (PC1 and PC2)")
dev.off() 

pdf(file="Q1_g2.pdf",width=10, height=10)
biplot(pr.stand.datas, scale=0, expand=1, choices = c(2, 3), col=c("#A6BDDB","black"), main="PCA (PC2 and PC3)")
dev.off() 

pdf(file="Q1_g3.pdf",width=10, height=10)
biplot(pr.stand.datas, scale=0, expand=1, choices = c(1, 3), col=c("#A6BDDB","black"), main="PCA (PC1 and PC3)")
dev.off() 
...
ic=ICr(datas, max.r = min(10, ncol(datas) - 1))
print(ic)
...
plot(cumsum(PVE_first_10), type = 'b', ylab = "cumulative PVE", ylim = c(0, 1), xlab = "PC", main = "cumulative variance explained by 10 PCs")
...
PVE_j = pr.new.var_j/sum(pr.new.var_j)

PVE_first_10_j <- PVE_j[1:10]

pdf("Q1_j.pdf",width=10, height=10)
plot(cumsum(PVE_first_10_j), type = 'b', ylab = "cumulative PVE", ylim = c(0, 1), xlab = "PC", main = "cumulative variance explained by 10 PCs (UK removed)")
text(x = 1:10, y = cumsum(PVE_first_10_j), labels = round(cumsum(PVE_first_10_j), 2), pos = 3, cex = 0.8)
dev.off()

######*****Question k

# calculate the standard errors of PC
# use the previous data with 10 PCs and UK included 
pc_se <- sqrt(pr.new.datas$sdev^2 / nrow(pr.new.datas$x))
conf_band <- qnorm(0.975) * pc_se

# the upper and lower bounds for the confidence bands
pc1_lower <- pr.new.datas$x[, 1] - conf_band[1]
pc1_upper <- pr.new.datas$x[, 1] + conf_band[1]
pc2_lower <- pr.new.datas$x[, 2] - conf_band[2]
pc2_upper <- pr.new.datas$x[, 2] + conf_band[2]

# plot the first two principal components with confidence bands

pdf("Q1_k.pdf",width=10, height=10)

plot(pr.new.datas$x[, 1], pr.new.datas$x[, 2], type = "n", xlab = "PC1", ylab = "PC2")
lines(pr.new.datas$x[, 1], pr.new.datas$x[, 2])
lines(pr.new.datas$x[, 1], pc2_lower, col = "red", lty = 2)
lines(pr.new.datas$x[, 1], pc2_upper, col = "red", lty = 2)
lines(pc1_lower, pr.new.datas$x[, 2], col = "blue", lty = 2)
lines(pc1_upper, pr.new.datas$x[, 2], col = "blue", lty = 2)
text(pr.new.datas$x[, 1], pr.new.datas$x[, 2], labels = 1:nrow(pr.new.datas$x), col = "black", pos = 3, cex = 0.7)
title('PC1 and PC2 over time with 95% confidence bands')
legend("bottomright", legend = paste("for PC", 1:2), col = c("blue", "red"),, pch = 19, bty = "n")
